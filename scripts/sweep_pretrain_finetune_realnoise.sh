#!/usr/bin/env bash
# Two-stage sim-pretrain (with real-noise injection) → nuScenes-finetune (B1+A1).
#
...
#
# Usage
#   bash scripts/sweep_pretrain_finetune_realnoise.sh                       # full pipeline
#   PRETRAIN_EPOCHS=10 FINETUNE_EPOCHS=30 bash scripts/sweep_pretrain_finetune_realnoise.sh
#   BATCH_SIZE=8 bash scripts/sweep_pretrain_finetune_realnoise.sh
#   SKIP_PRETRAIN=1 bash scripts/sweep_pretrain_finetune_realnoise.sh       # use existing pretrain ckpt
#   SKIP_FINETUNE=1 bash scripts/sweep_pretrain_finetune_realnoise.sh       # pretrain only (no FT, no eval)
#   WANDB=false bash scripts/sweep_pretrain_finetune_realnoise.sh
#   OUTPUT_ROOT=output bash scripts/sweep_pretrain_finetune_realnoise.sh
...
PRETRAIN_NAME="sim_pretrain_realnoise"
FINETUNE_NAME="pretrain_finetune_realnoise"
mkdir -p "$OUTPUT_ROOT"
...
{
  echo "════════════════════════════════════════════════════════════════"
  echo "  RadarTaco sim-pretrain (real-noise A1) → finetune sweep — $SWEEP_TAG"
...
if [ "$SKIP_PRETRAIN" -ne 1 ]; then
  PRE_ARGS="+experiment=sim_pretrain_finetune training=sim_pretrain dataset=mixed dataset.sim.ratio_real=0.0 dataset.sim.ratio_sim=1.0 dataset.sim.real_noise.enabled=true"
