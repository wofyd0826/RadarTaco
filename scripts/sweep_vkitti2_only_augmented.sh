...
#
# Modes
#   sim50_vkitti2_only_aug   real 50% + vKITTI2 50% (augmented radar — y-bias + noise, no semantic prior)
#   sim100_vkitti2_only_aug  vKITTI2 100% (augmented radar; val/test still nuScenes)
#
# Usage
#   bash scripts/sweep_vkitti2_only_augmented.sh                       # both, paper hparams
#   EPOCHS=15 bash scripts/sweep_vkitti2_only_augmented.sh             # cheaper epochs
#   BATCH_SIZE=8 bash scripts/sweep_vkitti2_only_augmented.sh          # smaller GPU
#   ONLY=sim50_vkitti2_only_aug bash scripts/sweep_vkitti2_only_augmented.sh   # subset
#   SKIP_TRAIN=1 bash scripts/sweep_vkitti2_only_augmented.sh          # eval-only (existing ckpts)
#   WANDB=false bash scripts/sweep_vkitti2_only_augmented.sh
#   OUTPUT_ROOT=output bash scripts/sweep_vkitti2_only_augmented.sh
...
mkdir -p "$OUTPUT_ROOT"
LOG="$OUTPUT_ROOT/_sweep_log_vkitti2_only_aug_${SWEEP_TAG}.log"

# ────────────────────────────────────────────────────── per-mode definitions ──
# format: NAME|hydra-args
declare -a EXPERIMENTS=(
  "sim50_vkitti2_only_aug|+experiment=mixed_with_aux dataset.sim.hypersim.enabled=false dataset.sim.ratio_real=0.5 dataset.sim.ratio_sim=0.5"
  "sim100_vkitti2_only_aug|+experiment=mixed_with_aux dataset.sim.hypersim.enabled=false dataset.sim.ratio_real=0.0 dataset.sim.ratio_sim=1.0"
)
...
{
  echo "════════════════════════════════════════════════════════════════"
  echo "  RadarTaco vKITTI2-only sweep (augmented radar) — $SWEEP_TAG"
...
# ───────────────────────────────────────────────────────── footer ──
{
  ...
  echo "  finished:    $(date)"
  echo "  full log:    $LOG"
  echo "  outputs:     $OUTPUT_ROOT/{sim50_vkitti2_only_aug,sim100_vkitti2_only_aug}/"
  echo "════════════════════════════════════════════════════════════════"
} | tee -a "$LOG"
